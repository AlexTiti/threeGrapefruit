...
public class BlueManager {
    ...
    public BlueManager(Context context) {
        ...
        android.bluetooth.BluetoothManager manager = (android.bluetooth.BluetoothManager) this.context
    ...
    public void requestBluetooth(Fragment context) {

            if (!bleAdapter.isEnabled()) {
                Intent enableBtIntent = new Intent(
                        BluetoothAdapter.ACTION_REQUEST_ENABLE);
                enableBtIntent.putExtra(BluetoothAdapter.EXTRA_DISCOVERABLE_DURATION, 200);
                context.startActivityForResult(enableBtIntent, REQUEST_CODE);
            }

    }

    public boolean isEnabledFragment() {
        if (bleAdapter == null) {
            return false;
        }

...
import com.findtech.threePomelos.musicserver.proxy.utils.Constants;
import com.findtech.threePomelos.musicserver.proxy.utils.MediaPlayerProxy;
import com.findtech.threePomelos.musicserver.proxy.utils.ProxyUtils;
...
public class MediaService extends Service {
    public static final String PLAYSTATE_CHANGED = "com.findtech.three.playstatechanged";
    public static final String POSITION_CHANGED = "com.findtech.three.positionchanged";
    public static final String META_CHANGED = "com.findtech.three.metachanged";
    public static final String QUEUE_CHANGED = "com.findtech.three.queuechanged";
    public static final String PLAYLIST_CHANGED = "com.findtech.three.playlistchanged";
    public static final String REPEATMODE_CHANGED = "com.findtech.three.repeatmodechanged";
    public static final String SHUFFLEMODE_CHANGED = "com.findtech.three.shufflemodechanged";
    public static final String TRACK_ERROR = "com.findtech.three.trackerror";
    public static final String TIMBER_PACKAGE_NAME = "com.findtech.three";
    public static final String MUSIC_PACKAGE_NAME = "com.android.music";
    public static final String SERVICECMD = "com.findtech.three.musicservicecommand";
    public static final String TOGGLEPAUSE_ACTION = "com.findtech.three.togglepause";
    public static final String PAUSE_ACTION = "com.findtech.three.pause";
    public static final String STOP_ACTION = "com.findtech.three.stop";
    public static final String PREVIOUS_ACTION = "com.findtech.three.previous";
    public static final String PREVIOUS_FORCE_ACTION = "com.findtech.three.previous.force";
    public static final String NEXT_ACTION = "com.findtech.three.next";
    public static final String MUSIC_CHANGED = "com.findtech.change_music";
    public static final String REPEAT_ACTION = "com.findtech.three.repeat";
    public static final String SHUFFLE_ACTION = "com.findtech.three.shuffle";
    public static final String FROM_MEDIA_BUTTON = "frommediabutton";
    public static final String REFRESH = "com.findtech.three.refresh";
    public static final String LRC_UPDATED = "com.findtech.three.updatelrc";
    ...
    public static final String TRACK_PREPARED = "com.findtech.three.prepared";
    public static final String BUFFER_UP = "com.findtech.three.bufferup";
    public static final String LOCK_SCREEN = "com.findtech.three.lock";
    public static final String SEND_PROGRESS = "com.findtech.three.progress";
    public static final String MUSIC_LODING = "com.findtech.three.loading";
    private static final String SHUTDOWN = "com.findtech.three.shutdown";
    public static final String SETQUEUE = "com.findtech.three.setqueue";
    ...
    private int removeTracksInternal(int first, int last) {
        L.e("SixItem", "removeTracksInternal==getNextPosition");
        synchronized (this) {
            if (last < first) {
                return 0;
            } else if (first < 0) {
            ...
            if (gotonext) {
                if (mPlaylist.size() == 0) {
                    ...
                    stop(false);
                    L.e("FFFFFFF", "openCurrentAndNext();");
                    openCurrentAndNext();
                    if (wasPlaying) {
                        play();
                    }
    ...
    private void addToPlayList(final long[] list, int position) {
        ...
        L.e("3.Success", mPlaylist.size() + "" + mPlayPos);
        if (mPlaylist.size() == 0) {
            closeCursor();
            notifyChange(META_CHANGED);
        }
    ...
    /**
     * 根局播放歌曲的id查询相应的Cursor
     * @param trackId
     */
    private void updateCursor(final long trackId) {
        L.e("FFFFFFF", "updateCursor===" + trackId);
    ...
    class RequestPlayUrl implements Runnable {
         ...
         * @param url
         */
        public RequestPlayUrl(long id, boolean play, String url) {
            this.id = id;
            this.play = play;
            this.url = url;
            L.e("FFFFFFF", "RequestPlayUrl == " + url);
        }
        ...
        public void run() {
            try {
                ...
                if (!stop) {
                    ...
                    File dir_size = new File(Constants.DOWNLOAD_PATH);
                    if (!dir_size.exists()) {
                        dir_size.mkdirs();
                    }
                    long freeSize = ProxyUtils.getAvailaleSize(Constants.DOWNLOAD_PATH);
                    if (freeSize >= Constants.SD_REMAIN_SIZE_MIN || (dir_size.exists() && dir_size.listFiles().length > 1)) {
                        startProxy();
                        String urlEn = url;
                        urlEn = mProxy.getProxyURL(urlEn);
                        mPlayer.setDataSource(urlEn);
                        L.e("FFFFFFF", "setDataSource == " + urlEn);
                    } else {
     ...
     * second
     */
    private void openCurrentAndNext() {//942
        L.e("======", "openCurrentAndNext");
     ...
     * @param openNext
     */
    private void openCurrentAndMaybeNext(final boolean play, final boolean openNext) {//947
        synchronized (this) {
            ...
            if (mPlaylist.size() == 0 || mPlaylistInfo.size() == 0 && mPlayPos >= mPlaylist.size()) {
                clearPlayInfos();
                L.e("FFFFFFF", "openCurrentAndNext()+clearPlayInfos");
                return;
            }
            final long id = mPlaylist.get(mPlayPos).mId;
            L.e("5", id + "openCurrentAndMaybeNext" + mPlayPos);
            updateCursor(id);
            ...
            if (!mPlaylistInfo.get(id).islocal) {
                ...
                mRequestUrl = new RequestPlayUrl(id, play, mPlaylistInfo.get(id).lrc);
                ...
                while (true) {
                    ...
                    if (mOpenFailedCounter++ < 10 && mPlaylist.size() > 1) {
                        ...
                        L.e(TAG, "Failed to open file for playback");
                        shutdown = true;
                        break;
                    }
                }
...
// * shutdown openNext
    ...
    private int getNextPosition(final boolean force) {
        ...
        if (!force && mRepeatMode == REPEAT_CURRENT) {
            ...
            int skip = SHUFFLER.nextInt(numTracksWithMinNumPlays);
            for (int i = 0; i < trackNumPlays.length; i++) {
                if (trackNumPlays[i] == minNumPlays) {
                    if (skip == 0) {
                        return i;
    ...
    private void setNextTrack() {
        L.e("FFFFFFF", "openNext===" + getNextPosition(false));
        ...
        if (mNextPlayPos >= 0 && mPlaylist != null && mNextPlayPos < mPlaylist.size()) {
            ...
            if (mPlaylistInfo.get(id) != null) {
                if (mPlaylistInfo.get(id).islocal) {
                    ...
                    long id_pre = getCurrentTrack().mId;
                    if (mPlaylistInfo.get(id_pre).islocal) {
    ...
    private void doAutoShuffleUpdate() {
        ...
        for (int i = 0; i < toAdd; i++) {
            ...
            int idx = -1;
            while (true) {
                idx = SHUFFLER.nextInt(mAutoShuffleList.length);
    ...
    private void notifyChange(final String what) {
        ...
        if (what.equals(META_CHANGED)) {
            ...
            saveQueue(true);
            if (isPlaying()) {
                L.e("SixItem", mNextPlayPos + "=" + mPlaylist.size() + "==" + SHUFFLE_NONE);
    ...
    private Notification getNotification() {
        ...
        remoteViews.setImageViewResource(R.id.iv_next, R.drawable.note_btn_next);
        remoteViews.setImageViewResource(R.id.iv_stop, R.drawable.note_btn_stop);
        ...
        nowPlayingIntent.setAction("com.findtech.three.LAUNCH_NOW_PLAYING_ACTION");
        nowPlayingIntent.setComponent(new ComponentName("com.findtech.threePomelos", "com.findtech.threePomelos.music.activity.PlayDetailActivity"));
        ...
        if (!isTrackLocal()) {
            if (mNoBit != null) {
                ...
                if (getAlbumPath() == null || uri == null) {
                    mNoBit = BitmapFactory.decodeResource(getResources(), R.drawable.icon_rhyme);
                    updateNotification();
                } else {
            ...
            remoteViews.setImageViewResource(R.id.image, R.drawable.icon_rhyme);
        }
    ...
    public void open(final HashMap<Long, MusicInfo> infos, final long[] list, final int position) {
        synchronized (this) {
            L.e("3.Success", mPlaylist.size() + "open" + mPlayPos);
            mPlaylistInfo = infos;
            ...
            if (newlist) {
                //添加新的播放队列
                L.e("3.Success", newlist + "open" + mPlayPos);
            ...
            if (position >= 0) {
                ...
                //随机位置
                mPlayPos = SHUFFLER.nextInt(mPlaylist.size());
            }
            //清除历史数据
            mHistory.clear();
            //耗时操作
            L.e("=====", "======openCurrentAndNextPlay");
            openCurrentAndNextPlay(true);
            if (oldId != getAudioId()) {
                L.e("FFFFFFE", oldId + "oldId != getAudioId() " + getAudioId());
                notifyChange(META_CHANGED);
            }
        }
    }
    ...
    public void play() {//2178
        L.e("9", "public void play()=====================================================");
        play(true);
    }
    ...
    public void play(boolean createNewNextTrack) {
        L.e("FFFFFFF", "createNewNextTrack" + createNewNextTrack);
        ...
        if (createNewNextTrack) {
            ...
            setNextTrack(mNextPlayPos);
            L.e("FFFFFFF", "createNewNextTrack else" + mNextPlayPos);
        }

        if (mPlayer.isTrackPrepared()) {
            final long duration = mPlayer.duration();
            L.e("start()", " mPlayer.start()" + mPlayer.isTrackPrepared());
            if (mRepeatMode != REPEAT_CURRENT && duration > 2000
                    && mPlayer.position() >= duration - 2000) {
                gotoNext(true);
            }
        }
        boolean b = mPlayer == null;
        L.e("start()", " mPlayer.start()" + b);
        mPlayer.start();
        L.e("start()", "After mPlayer.start()");
    ...
    private static final class MultiPlayer implements MediaPlayer.OnErrorListener,
        ...
        public void setNextDataSource(final String path) {
            ...
            L.e("mNextMediaPlayer", " not=======null");

            if (setNextDataSourceImpl(mNextMediaPlayer, path)) {
                L.e("mNextMediaPlayer", "setNextDataSourceImpl");
                mNextMediaPath = path;
        ...
        private boolean setDataSourceImpl(final MediaPlayer player, final String path) {
            L.e("start()=======", "setDataSourceImpl" + mIsTrackNet);
            mIsTrackNet = false;
            mIsTrackPrepared = false;
            L.e("start()=======", "setDataSourceImpl" + path);
            try {
                player.reset();
                ...
                if (path.startsWith("content://")) {
                    ...
                    L.e("start()=======", mIsTrackNet + "setDataSourceImpl" + path);
                } else {
        ...
        private boolean setNextDataSourceImpl(final MediaPlayer player, final String path) {
            L.e("SixItem", "setNextDataSourceImpl");
            ...
            try {
                ...
                player.setAudioStreamType(AudioManager.STREAM_MUSIC);
                if (path.startsWith("content://")) {
                    L.e("SixItem", "setOnPreparedListener");
                    //设置准备监听事件
        ...
        public void start() {
            ...
            if (!mIsTrackNet) {
                L.e("start()==========", "mIsTrackNet" + mIsTrackNet);
        ...
        Runnable startMediaPlayerIfPrepared = new Runnable() {
            ...
            @Override
            public void run() {
                L.e("start()", "mIsTrackNet" + mIsTrackPrepared);
                if (mIsTrackPrepared) {
                    mCurrentMediaPlayer.start();
                    final long duration = duration();
